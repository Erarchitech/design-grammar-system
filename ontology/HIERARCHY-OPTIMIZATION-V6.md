...
> Analysis of why Protégé shows ~134 ungrouped external classes after loading `[DesignGrammar-V6.owl](DesignGrammar-V6.owl)` v3.3 with its 7 imports, and ranked options for cleaning up the hierarchy view.
...
**Compiled:** 2026-05-15
**Scope:** Protégé class hierarchy view of DesignGrammar-V6.owl with all 7 cross-ontology imports resolved (PROV-O, SOSA, SHACL, SKOS, DCTerms, GeoSPARQL, SWRL).
...
**What:** Refactor into:
- **`DesignGrammar-V6.owl`** — DG content only. No `owl:imports`. No cross-vocab alignment axioms. Pure DG ontology with Core band + 5 layer hubs.
- **`DesignGrammar-standards-extension-V6.owl`** — A *facade* ontology containing only:
  - `owl:imports DesignGrammar-V6.owl`
...
**Trade-offs:**
- ✅ **Clean Protégé view by default** — opening `DesignGrammar-V6.owl` shows only DG classes.
- ✅ **Full alignment preserved** — opening `DesignGrammar-standards-extension-V6.owl` brings in everything.
...
```
ontology/
├── DesignGrammar-V6.owl              ← DG core (lean, ~150 KB → ~120 KB after stripping alignments)
├── DesignGrammar-standards-extension-V6.owl      ← Alignment facade (~20 KB, mostly imports + axioms)
└── catalog-v001-V6.xml               ← Maps both IRIs + 7 externals
```

`DesignGrammar-standards-extension-V6.owl` looks like:
...
2. For each external vocab, extract a SLME (Semantic Locality Module Extraction) module:
   ...
   ```
3. Update `catalog-v001-V6.xml` to redirect each import to its local module file.
...
2. **Workflow-appropriate.** When you're working on DG itself (adding classes, refining axioms), you load `DesignGrammar-V6.owl` — clean. When you're auditing alignments or integrating with PROV-aware tooling, you load `DesignGrammar-standards-extension-V6.owl`.
...
4. **Composable for downstream.** A downstream consumer who wants only DG can import `DesignGrammar-V6.owl`. A consumer building a full PROV/SHACL/SOSA-integrated knowledge graph imports `DesignGrammar-standards-extension-V6.owl`. Same ontology, two entry points.
...
1. **Create `[DesignGrammar-standards-extension-V6.owl](DesignGrammar-standards-extension-V6.owl)`** — new file with header, 1+7 imports, and the 49 alignment axioms.
2. **Strip from `[DesignGrammar-V6.owl](DesignGrammar-V6.owl)`:**
...
3. **Update `[catalog-v001-V6.xml](catalog-v001-V6.xml)`** — add entry mapping `http://example.org/design-grammar/aligned` → `DesignGrammar-standards-extension-V6.owl`.
4. **Update `[ONTOLOGY-ALIGNMENT.md](ONTOLOGY-ALIGNMENT.md)`** — document the new two-file pattern, explain when to load which.
5. **Regenerate `[DesignGrammar-V6.md](DesignGrammar-V6.md)`** — auto-export now reflects clean structure.
