// updated_cypher_reference_examples_v4.cypher
// Schema version: v4.0
//
// Few-shot reference examples for LLM Cypher generation (ingest path).
// Mirrors the v4 CYPHER TEMPLATE BLOCK from cypher_template.txt.
// Each example demonstrates a complete Rule subgraph: OntoGraph terms +
// Metagraph rule/atom/var/literal/builtin nodes + relationships.
//
// NOTE: DesignState and Run nodes are NOT emitted by LLM ingest — they
// are created by VALIDATOR-on-publish only (see cypher_template.txt v4
// GRAPH SCHEMA reference section for the document-only schema).
// This file emits OntoGraph + Metagraph nodes only.
//
// Property naming (PascalCase per D-05):
//   r.SWRL         (was r.text)
//   r.RuleName     (was r.title, optional)
//   r.RuleDescription (new, optional)
//   r.kind         (unchanged)

// === Example 1: Maximum height of each building must be at most 75 m. ===
//
// Demonstrates the basic violation pattern with r.SWRL (no RuleName/
// RuleDescription — illustrates the minimal v4 Rule node).
//
// Rule_Id: R_URB_HEIGHT_MAX_75_V
...
SET r.SWRL = 'Building(?b)^hasHeightM(?b,?h)^swrlb:greaterThan(?h,75)->violatesMaxHeight(?b,true)',
    r.kind = 'violation',
    r.project = 'default-project',
    r.graph = 'Metagraph'
    // r.RuleName and r.RuleDescription omitted — both are optional
...
// === Example 2: Minimum Hours of Direct Sunlight for Each Building ===
// must be at least 2.8 hours per day.
//
// Demonstrates a Rule node with r.RuleName (optional name property).
// Violation pattern: body checks values less than 2.8, head sets flag.
//
// Rule_Id: R_SUNLIGHT_MIN_2_8H_V
...
SET r.SWRL = 'Building(?b)^directSunlightHoursPerDay(?b,?s)^swrlb:lessThan(?s,2.8)->violatesMinDirectSunlight(?b,true)',
    r.kind = 'violation',
    r.RuleName = 'Minimum Hours of Direct Sunlight for Each Building must be at least 2.8 hours/day (violation if less than 2.8)',
    r.project = 'default-project',
    r.graph = 'Metagraph'
    // r.RuleDescription omitted — optional, LLM may set it when the NL
    // rule text supplies a description
...
// === Example 3: Urban block grid size must be between 12 m and 48 m ===
// (violation if less than 12 m — min-bound rule).
//
// Demonstrates a rule with no RuleName or RuleDescription —
// only the mandatory r.SWRL + r.kind are set.
//
// Rule_Id: R_URBANBLOCK_GRID_MIN_12M_V
...
SET r.SWRL = 'UrbanBlock(?ub)^blockGridSizeM(?ub,?g)^swrlb:lessThan(?g,12)->violatesMinUrbanBlockGridSize(?ub,true)',
    r.kind = 'violation',
    r.project = 'default-project',
    r.graph = 'Metagraph'
    // r.RuleName and r.RuleDescription omitted — both are optional
...
// === Example 4: Window-to-wall ratio for each facade must not exceed 40%. ===
//
// NEW in v4 — demonstrates the full Rule property set including both
// r.RuleName and r.RuleDescription (the optional descriptive properties).
// Shows how the LLM populates optional fields when the NL rule text
// supplies a name and description.
//
// Rule_Id: R_ENERGY_WWR_MAX_40_V

MERGE (C_Facade:Class {iri: 'ex:Facade'})
SET C_Facade.label = 'Facade',
    C_Facade.project = 'default-project',
    C_Facade.graph = 'OntoGraph'

MERGE (DP_WWR:DatatypeProperty {iri: 'ex:hasWWR'})
SET DP_WWR.SWRL_label = 'hasWWR',
    DP_WWR.range = 'xsd:decimal',
    DP_WWR.project = 'default-project',
    DP_WWR.graph = 'OntoGraph'
...
MERGE (DP_V:DatatypeProperty {iri: 'ex:violatesMaxWWR'})
SET DP_V.SWRL_label = 'violatesMaxWWR',
    DP_V.range = 'xsd:boolean',
    DP_V.project = 'default-project',
    DP_V.graph = 'OntoGraph'

MERGE (r:Rule {Rule_Id: 'R_ENERGY_WWR_MAX_40_V'})
SET r.SWRL = 'Facade(?f)^hasWWR(?f,?w)^swrlb:greaterThan(?w,0.4)->violatesMaxWWR(?f,true)',
    r.kind = 'violation',
    r.RuleName = 'Maximum Window-to-Wall Ratio',
    r.RuleDescription = 'Facades must have a window-to-wall ratio of at most 40% to control solar heat gain per building energy code',
    r.project = 'default-project',
    r.graph = 'Metagraph'

MERGE (vf:Var {name: '?f', project: 'default-project'})
SET vf.graph = 'Metagraph'

MERGE (vw:Var {name: '?w', project: 'default-project'})
SET vw.graph = 'Metagraph'

MERGE (lit04:Literal {lex: '0.4', datatype: 'xsd:decimal'})
SET lit04.project = 'default-project',
    lit04.graph = 'Metagraph'

MERGE (litT:Literal {lex: 'true', datatype: 'xsd:boolean'})
SET litT.project = 'default-project',
    litT.graph = 'Metagraph'

MERGE (a1:Atom {Atom_Id: 'R_ENERGY_WWR_MAX_40_V_A1'})
SET a1.type = 'ClassAtom',
    a1.iri = 'ex:Facade',
    a1.SWRL_label = 'Facade',
    a1.project = 'default-project',
    a1.graph = 'Metagraph'

MERGE (a2:Atom {Atom_Id: 'R_ENERGY_WWR_MAX_40_V_A2'})
SET a2.type = 'DataPropertyAtom',
    a2.iri = 'ex:hasWWR',
    a2.SWRL_label = 'hasWWR',
    a2.project = 'default-project',
    a2.graph = 'Metagraph'

MERGE (a3:Atom {Atom_Id: 'R_ENERGY_WWR_MAX_40_V_A3'})
SET a3.type = 'BuiltinAtom',
    a3.iri = 'swrlb:greaterThan',
    a3.SWRL_label = 'greaterThan',
    a3.project = 'default-project',
...
MERGE (h1:Atom {Atom_Id: 'R_ENERGY_WWR_MAX_40_V_H1'})
SET h1.type = 'DataPropertyAtom',
    h1.iri = 'ex:violatesMaxWWR',
    h1.SWRL_label = 'violatesMaxWWR',
    h1.project = 'default-project',
    h1.graph = 'Metagraph'
...
MERGE (a1)-[:REFERS_TO]->(C_Facade)
MERGE (a2)-[:REFERS_TO]->(DP_WWR)
MERGE (a3)-[:REFERS_TO]->(B_gt)
MERGE (h1)-[:REFERS_TO]->(DP_V)

MERGE (a1)-[:ARG {`pos`: 1}]->(vf)
MERGE (a2)-[:ARG {`pos`: 1}]->(vf)
MERGE (a2)-[:ARG {`pos`: 2}]->(vw)
MERGE (a3)-[:ARG {`pos`: 1}]->(vw)
MERGE (a3)-[:ARG {`pos`: 2}]->(lit04)
MERGE (h1)-[:ARG {`pos`: 1}]->(vf)
MERGE (h1)-[:ARG {`pos`: 2}]->(litT);
