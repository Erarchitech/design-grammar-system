...
**Planned:** 2026-07-03
**Restructured:** 2026-07-08 — Grasshopper canvas → Computgraph serialization elaborated: former Phase 5 (GH node recognition) expanded into Phases 5–10; former Phases 6/7/8 renumbered to 11/12/13. Script *generation/editing/consulting* deferred to the v10 seed (`.planning/milestones/v10.0-SEED.md`).
**Status:** Paused (Phase 1 executed 2026-07-06, parked; resume after v8.0 follow-ups)
**Depends on:** v7.0 (VALIDATOR rework, composed DesignState, PARAMETER REINSTATE, graph schema v4 — shipped 2026-07-05). v8.0 shipped 2026-07-07 — all UI deliverables in this roadmap target **`ui-v2/`** (the legacy `graph-viewer/` SPA is archived; Phase 1's settings panel predates the cutover and is revisited in Phase 13 docs pass).

> **Isolation contract:** This document and `v9.0-REQUIREMENTS.md` are the primary v9.0 artifacts. Nothing here modifies `.planning/phases/`, STATE.md, or shipped milestones. Phase numbers are milestone-local (1–13).

## Overview

v9.0 upgrades the intelligence layer of the Design Grammar System along two axes:

1. **LLM infrastructure** — a provider-agnostic **cloud LLM connector** (Phase 1, ✅ built): the user enters an API key in the UI and controls which LLM (Anthropic Claude, OpenAI-compatible, or local Ollama fallback) manages Design Grammars. On top sits a **DG-aware context layer** (Phase 2) making every LLM call ontology-aware, and the rules ingest/edit pipeline is rebuilt on it (Phase 4), with an orchestration decision gate (Phase 3).
...
2. **Grasshopper canvas intelligence** — the ontology's **Computgraph layer** (`dgc:` in `ontology/DesignGrammar-V7.owl`) gets its first runtime consumer. The graph context of a Grasshopper script is serialized to Neo4j as `Object → Behavior → Algorithm → Procedure → Pattern → Parameter/Interface`, via a pipeline the architect controls end-to-end on the canvas:
   - **Serialization core** (Phase 5): GH_Document traversal + the DG Canvas Annotation Convention parser (`OBJECT - FRAME`, `1_ALGORITHM`, `11_Proc`, `11_Pat_*`, `11_Var_/Const_/Emg_/IntF_*` — the Frame worked example, already modeled as OWL named individuals).
   - **DG Canvas Bridge** (Phase 6): a native adaptation of the [grasshopper-mcp](https://github.com/alfredatnycu/grasshopper-mcp) pattern — a TCP listener component in the DG plugin + a bridge client and `gh_*` MCP tools in data-service.
   - **Tagging components** (Phase 7): the architect marks known ontological entities on canvas (components + manual selection).
   - **LLM recognition + on-canvas preview** (Phase 8): AI guesses the untagged rest; the proposal appears as temporary groups/scribbles on the canvas for confirmation **before anything is published**.
   - **Persistence + display** (Phase 9): MERGE-idempotent Computgraph writes, project-isolated, with provenance; ui-v2 Computgraph layer view.
   - **Structure validation MVP** (Phase 10): Design Rules mapped to script structure — deterministic Cypher checks + an LLM consult endpoint. This is the seam v10 Script Intelligence (generate/edit/consult) builds on.

Plus **AI-generated script inputs** (Phase 11, rides Computgraph parameters), the **DesignState auto-validation investigation** (Phase 12), and **E2E + docs** (Phase 13).

**13 phases, estimated ~16–20 implementation sessions.**
...
- LLM settings panel (built in legacy `graph-viewer/index.html` pre-v8.0 cutover — ui-v2 port tracked in Phase 13)
- n8n workflows rewired to the gateway; pytest coverage

**Success Criteria:** met — see `v9.0-phases/01-cloud-llm-connector/01-VERIFICATION.md`

**Plans:** 3 plans (complete)
- [x] 01-01-PLAN.md — Gateway Core
- [x] 01-02-PLAN.md — n8n Workflow Rewiring
- [x] 01-03-PLAN.md — UI Settings Panel
...
- Computgraph concept catalog block (dgc: classes, relations, enum values, the annotation-convention grammar) — consumed by Phase 8 recognition prompts
...
**Requires:** Phase 1. Can run parallel to Phase 2.
...
### Phase 5: Computgraph Serialization Core
...
### Phase 6: DG Canvas Bridge (grasshopper-mcp adaptation)
...
**Requires:** Phase 5 (the listener serves `cgContextJson`).

**Requirements:** BRDG-01, BRDG-02, BRDG-03, BRDG-04

**Deliverables:**
- `DG/src/DG.Grasshopper/Components/CanvasListenerComponent.cs` — **DG CANVAS LISTENER**: `TcpListener` on a configurable port (default **8720**), grasshopper-mcp wire protocol (newline-terminated JSON `{"type": ..., "parameters": {...}}`, UTF-8); commands: `get_canvas_context` (Phase 5 serializer), `get_selection` (selected object ids), `preview_structure` / `clear_preview` / `get_preview_status` (Phase 8 consumers); canvas access marshalled via `RhinoApp.InvokeOnUiThread`; on/off boolean input; status output
...
### Phase 7: Ontology Tagging Components and Manual Selection
...
**Requires:** Phase 5 (convention grammar + styles are the contract). Parallel-safe with Phase 6.
...
**Success Criteria:**
1. Selecting a slider and tagging it as Var with name "SpansCount" under procedure 11 produces a pink group named `11_Var_SpansCount` — and the Phase 5 serializer reads it back as a `CgParameter` (Variable)
...
### Phase 8: LLM Recognition and On-Canvas Proposal Preview
...
**Requires:** Phases 2 (Computgraph concept catalog in context assembler), 6 (bridge), 7 (tags as anchors).

**Requirements:** RCGN-01, RCGN-02, RCGN-03, RCGN-04

**Deliverables:**
- `data-service/cg_recognition.py` — recognition pipeline: `cgContextJson` (tagged anchors + untagged nodes/wires) + Computgraph concept catalog + Frame few-shot example → LLM gateway → **proposed-structure JSON** (strict schema: entity kind, name suggestion, member component ids, confidence, rationale); schema-validated with bounded retry via the Phase 2 validator pattern; `POST /computgraph/recognize`
...
- Confirm/reject flow: `DG/src/DG.Grasshopper/Components/StructureConfirmComponent.cs` — **DG STRUCTURE CONFIRM**: lists pending proposals per procedure; accept converts preview groups to permanent convention groups (Phase 7 styles), reject removes them cleanly; partial accept supported; `clear_preview` wipes all pending
...
3. Accepting proposal(s) yields permanent groups that the Phase 5 serializer parses identically to hand-made tags (`source: recognized` recorded)
...
### Phase 9: Computgraph Persistence and Graph Layer Display
...
**Requires:** Phases 5 (serializer), 8 (confirmed structures; direct tagged-only publish works without 8).
...
- Schema propagation checklist: `cypher_template.txt`, `training/dataset_schema.json`, `spec/DATABASE.md`, `CLAUDE.md` schema tables, n8n/orchestrator prompts via the Phase 2 catalog
...
### Phase 11: AI-Generated Grasshopper Script Inputs
...
**Requires:** Phase 9 (`dgc:` Parameter structure with slider domains comes from serialization + persistence); v7.0 PARAMETER REINSTATE (milestone prerequisite).
...
### Phase 12: DesignState Auto-Validation Investigation
...
**Requires:** v7.0 VALIDATOR + composed DesignState shipped (milestone prerequisite). Independent of Phases 3–11; can run parallel.
...
### Phase 13: E2E Validation and Docs
...
**Requires:** Phases 1–12.

**Requirements:** INTG-01, INTG-02, INTG-03, INTG-04

**Deliverables:**
- E2E run: NL rule → cloud-LLM ingest (context layer + validation) → graph → GH validation → Speckle publish, on live Docker
- E2E GH intelligence chain: object marked + entities tagged → LLM recognition → on-canvas preview → confirm → Computgraph publish → structure validation → generated inputs accepted → applied via PARAMETER REINSTATE → validation run (auto or manual per Phase 12 outcome)
...
- Docs: CLAUDE.md service map + schema tables + gotchas, `spec/DATABASE.md` Computgraph section, DG_OBSIDIAN notes (decisions from Phases 3 and 12 filed; DG Canvas Annotation Convention note), `docs/` component reference for the 4–5 new GH components, graphify refresh
...
|-------|----------------|--------|-----------|
| 1. Cloud LLM Connector and Provider Abstraction | 3/3 | Complete | 2026-07-06 |
...
| 5. Computgraph Serialization Core | 0/? | Not started | — |
| 6. DG Canvas Bridge (grasshopper-mcp adaptation) | 0/? | Not started | — |
| 7. Ontology Tagging Components and Manual Selection | 0/? | Not started | — |
| 8. LLM Recognition and On-Canvas Proposal Preview | 0/? | Not started | — |
| 9. Computgraph Persistence and Graph Layer Display | 0/? | Not started | — |
| 10. Script Structure Validation MVP | 0/? | Not started | — |
| 11. AI-Generated Grasshopper Script Inputs | 0/? | Not started | — |
| 12. DesignState Auto-Validation Investigation | 0/? | Not started | — |
| 13. E2E Validation and Docs | 0/? | Not started | — |

Dependency shape: `1 → 2 → 4` (4 also gated by 3's ADR); `1 → 3` (parallel to 2); `5 → 6 → 8`; `5 → 7 → 8` (6 ‖ 7); `2 → 8`; `{5,8} → 9 → 10`; `9 → 11`; `12` parallel to 3–11; all → `13`. **Phase 5 has no v9.0 dependencies and can start immediately.**
...
| File | Phase | Change |
|------|-------|--------|
| `data-service/llm_gateway.py` | 1 ✅ | Provider adapters (Claude / OpenAI-compatible / Ollama) |
| `data-service/dg_context.py` | 2, 8, 10 | **New** — deterministic ontology/SWRL/Cypher/Computgraph context assembler |
| `llm/cypher_catalog.json` | 2 | **New** — versioned standard Cypher expression catalog |
| `data-service/gh_bridge.py` | 6 | **New** — TCP client to DG CANVAS LISTENER (host.docker.internal:8720) |
| `data-service/cg_recognition.py` | 8 | **New** — LLM recognition pipeline + proposed-structure schema |
| `data-service/cg_structure_checks.py` | 10 | **New** — deterministic structural checks + rule-mapped checks |
| `data-service/app.py` | 2, 6, 8, 9, 10, 11 | Context debug endpoint; `/computgraph/context/pull`, `gh_*` MCP tools; `/computgraph/recognize`; `/computgraph/publish`; `/computgraph/validate` + `/consult`; input generation |
| `docker-compose.yml` | 3, 6 | OpenClaw spike container; `GH_BRIDGE_*` env + `extra_hosts` |
| `DG/src/DG.Core/Models/Computgraph/*` | 5 | **New** — GH-free Computgraph object model |
| `DG/src/DG.Core/Parsing/CanvasAnnotationParser.cs` | 5 | **New** — annotation-convention grammar |
| `DG/src/DG.Core/Serialization/ComputgraphContextSerializer.cs` | 5 | **New** — cgContextJson v1 |
| `DG/src/DG.Grasshopper/Canvas/CanvasContextExtractor.cs` | 5 | **New** — GH_Document traversal |
| `DG/src/DG.Grasshopper/Canvas/CanvasAnnotationStyles.cs` | 7 | **New** — per-kind group colors/styles |
| `DG/src/DG.Grasshopper/Components/CanvasListenerComponent.cs` | 6 | **New** — DG CANVAS LISTENER (TCP bridge) |
| `DG/src/DG.Grasshopper/Components/ObjectMarkerComponent.cs` | 7 | **New** — DG OBJECT MARKER |
| `DG/src/DG.Grasshopper/Components/EntityTagComponent.cs` | 7 | **New** — DG ENTITY TAG |
| `DG/src/DG.Grasshopper/Components/StructureConfirmComponent.cs` | 8 | **New** — DG STRUCTURE CONFIRM (+ publish trigger, Phase 9) |
| `DG/tests/DG.Tests/` | 5, 7, 8 | Frame fixture; parser/serializer/proposal-schema tests |
| `ui-v2/src/graph/`, `ui-v2/src/screens/` | 4, 9, 11 | Edit-preview UI; Computgraph layer styling/filter; input candidate review |
| `n8n/workflows/*.json` | 2, 4 | Thin context-assembler callers; upgraded ingest/edit flow (or OpenClaw successor per Phase 3 ADR) |
| `cypher_template.txt`, `training/dataset_schema.json`, `spec/DATABASE.md` | 9 | Computgraph labels/relations (standard propagation checklist) |

---

*Roadmap defined: 2026-07-03; restructured 2026-07-08 — Computgraph serialization pipeline (Phases 5–10) elaborated from the Frame worked example; script generation/editing/consulting seeded for v10*
