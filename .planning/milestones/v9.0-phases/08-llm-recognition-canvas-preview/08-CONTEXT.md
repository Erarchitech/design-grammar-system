# Phase 8 Context: LLM Recognition and On-Canvas Proposal Preview

**Milestone:** v9.0 AI Workflow Intelligence (restructured 2026-07-08)
**Requirements:** RCGN-01..04
**Depends on:** Phase 2 (Computgraph concept catalog in `dg_context.py`), Phase 6 (bridge preview commands), Phase 7 (tags as anchors).
...
1. **`data-service/cg_recognition.py`** + `POST /computgraph/recognize`:
   ...
   - Output: **proposed-structure JSON** (contract in `../05-computgraph-serialization-core/05-RESEARCH.md` §6): per proposal `kind`, `suggestedName` (convention-conformant), `procedureIndex`, `memberIds`, `confidence`, `rationale`; plus `unrecognized[]` with reasons
...
3. **DG STRUCTURE CONFIRM** (`Components/StructureConfirmComponent.cs`):
   ...
   - Accept → the preview group is restyled/renamed into a **permanent convention group** (identical to a Phase 7 manual tag) and marked `source: recognized` in the listener's annotation registry; Reject → group removed cleanly
   - Partial accept supported; leftover proposals stay pending
   - Nothing in this flow writes to Neo4j — publish is Phase 9, and it only ever sees confirmed structure
...
- Confidence and rationale are mandatory per proposal (auditable AI, feeds provenance in Phase 9).
