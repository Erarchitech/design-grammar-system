# ptBIM 2026 — PowerPoint Editing Instructions (R3)
...
## NEW SLIDE D — "What we mean by 'Design State'"
**Insert immediately after New Slide C (Design Knowledge), before the Hook.** This is a brief definition slide — it completes the conceptual pair introduced by Slide C: if Design Knowledge is *the rules*, Design State is *the thing being tested*. Use the same content layout as Slide C: title top-left, one lead definition set larger, then a short breakdown below.

Title: «What we mean by "Design State"»

**Lead definition** (set larger / emphasised, same style as New Slide C lead):

«A Design State is a uniquely identified snapshot of a design's current parametric configuration — the set of dimensional parameters and object–geometry assignments captured from the parametric tool at a specific point in time. It is the *thing a Rule evaluates*: the target of compliance checking, not the rule itself and not the finished building model.»

**Two subtypes** (compact bullets; grounded in the ontology):

«• **DefState** — the parameter vector: the numeric, integer, and boolean values from Grasshopper sliders and toggles at the moment of capture. Identified by a SHA-256 StateId derived from the parameter values. When any slider changes, the StateId changes and dependent geometry identifiers regenerate. Prefix: `DS_`.»

«• **ObjectState** — the object assignment: maps SWRL variable references (from rule ClassAtoms) to geometry elements in the model. Combined with DefState to produce stable per-element identifiers (`IdRef`) used for cross-run tracking. Prefix: `OS_`.»

**Relation to Design Knowledge and BIM** (two compact lines or stacked bullets):

«A Metagraph **Rule** (Design Knowledge) `validates` a **DesignState** — the rule fires when its constraint is evaluated against this snapshot. Design Knowledge says *what must hold*; Design State provides *what currently is*.»

«IFC/BIM records the final delivered artefact — objects, relationships and properties at handover. A Design State records a configuration *during design*, before delivery — enabling compliance checking at any iteration, not only at the model submission stage.»

**Caption / one-liner for the speaker (bottom of slide, small):**

«In FBS terms: IFC carries the building's **Structure**. Design Knowledge carries its **Function** and rules. Design State is the testable **Behaviour** — the parametric configuration the rules are evaluated against at a given moment.»
...
1. **New Slide D** (Design State definition) — completes the conceptual pair: Knowledge is the rules, State is the thing being tested. Insert immediately after New Slide C.
...
- [ ] New Slides C, D, A and B match content-slide layout and numbering updated.
...
- [ ] New Slide D (Design State definition) inserted immediately after New Slide C; DefState / ObjectState subtypes shown; FBS caption links State to Knowledge and Structure correctly.
