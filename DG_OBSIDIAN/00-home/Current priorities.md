...
1. **v8.2 Connector Integration & Reasoning Engine — Phases 820-824** — **Phase 822 context gathered (2026-07-12).** 4 серых зоны обсуждены, 13 решений (D-01…D-13) в `822-CONTEXT.md`: result expands in HermiT card, unsat-class count + labeled list (no raw IRIs), four distinct verdict states (Consistent/Inconsistent/Unknown/Cancelled) with separate error line, "Last checked" timestamp + in-session persistence, per-card Run on HermiT only. IRI→label resolution location flagged for research (sidecar-enrich / proxy-resolve / UI-resolve). See [[sessions/2026-07-12 Phase 822 context — OWL 2 DL Reasoning Integration and Reasoner Screen Wiring|session]] and [[decisions/reasoner-screen-phases-checks-results|decision]]. **Next: `/gsd-plan-phase 822`.** Phase 821 context gathered (2026-07-11): custom Cypher+RDFLib, sidecar owns pipeline, sync API + hard timeout, hybrid union + disjointness overlay, fidelity tests + drift-immunity guard. See [[sessions/2026-07-11 Phase 821 context discussion — dg-reasoner sidecar and RDF translation|session]]. Phase 820 complete (3/3 plans, 12/12 must-haves): hybrid axiom-scoping + sidecar confirmed, spec at `spec/LPG-OWL-MAPPING.md`.
