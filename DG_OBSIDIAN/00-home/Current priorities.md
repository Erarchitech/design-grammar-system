...
1. **v9.0 AI Workflow Intelligence — Phases 28-40 (+32.1, +33, +34, +35, +36)** — Phase 36 "Computgraph Persistence and Graph Layer Display" ✅ COMPLETE (2026-07-19). Autonomous execution: 4 plans in 1 wave, 12 code/doc commits + 1 fix commit. Delivered: `POST /computgraph/publish` endpoint with MERGE-idempotent Cypher (atomic one-transaction), DG COMPUTGRAPH PUBLISH Grasshopper component (rising-edge trigger), ui-v2 Computgraph layer display (7 labels/3 orbits, casing fix, 200-char contextJson truncation), schema propagation across 8 surfaces. Verification: 15/15 must-haves, Python 5/5, .NET build ✓, Vite build ✓. 3 doc bugs caught post-verification (HAS_INTERFACE/PARAM_LINK directions, Algorithm merge key — runtime was correct, docs were stale). See [[sessions/2026-07-19 Phase 36 Computgraph Persistence and Graph Layer Display|session]] and [[decisions/Phase 36 Computgraph publish avoids mint_identity|decision]].
