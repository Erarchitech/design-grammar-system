...
1. **v8.2 Connector Integration & Reasoning Engine — Phases 820-824** — Initialized (2026-07-11), ready to plan. Rewires the Grasshopper CONNECTOR component to platform-issued credentials (CONNG-01/02) and replaces the v8.1 Reasoner placeholder with real OWL 2 DL (HermiT, via a new isolated `dg-reasoner` sidecar) + SHACL validation (REAS-04/05/06, SHCL-01/02). Phase 820 (architecture decision + OntoGraph axiom scoping) gates everything else — DG's live ontology has no real RDF axioms today, must be resolved before any reasoner code lands. 7/7 requirements mapped. See [[sessions/2026-07-11 v8.2 milestone init — Connector Integration and Reasoning Engine|session]] and [[decisions/Reasoning runs in isolated dg-reasoner sidecar not embedded in data-service|sidecar decision]]. Next: `/gsd-plan-phase 820`.
